...
from .utils.note_util import (
    beautify_content,
    sort_note_by_headings,
    preprocess_note_title_list,
)
...
class ETAPI:
    ...
    def search_note(self, search: str, **params) -> dict:
        ...
        res = requests.get(
            url, params=format_query_string(params), headers=self.get_header()
        )
        return res.json()
    ...
    def patch_note(
        ...
        if dateCreated or utcDateCreated:
            formatted_date_created, formatted_utc_date_created = format_dates_for_api(
                local_date=dateCreated, utc_date=utcDateCreated
            )
    ...
    def patch_branch(
        self, branchId: str, notePosition: int, prefix: str, isExpanded: bool
    ) -> dict:
        url = f"{self.server_url}/etapi/branches/{branchId}"
        params = {
            "notePosition": notePosition,
    ...
    def add_todo(
        ...
        try:
            ...
            if not todo_labels:
                ...
                todo_item_html = ListTemplate(todo_caption).substitute(
                    items=todo_item_html
                )
    ...
    def update_todo(self, todo_index: int, todo_description: str) -> bool:
        ...
        try:
            ...
            target_span = todo_label.find_next(
    ...
    def upload_md_file(self, file: str, parentNoteId: str, parse_math: bool = True):
        ...
        if images:
            ...
            for match in images:
                ...
                if os.path.isdir(image_file_path) or os.path.isdir(
                    image_file_path_unquote
                ):
                    continue
    ...
    def create_attachment(
        ...
        res = requests.post(
            url, data=clean_param(params), headers=self.get_header()
        ).json()
    ...
    def optimize_image_attachments(self, noteId: str, quality: int = 90):
        ...
        for attachment in attachments:
            try:
                ...
                if (
                    not attachment["role"] == "image"
                    and attachment["contentLength"] > 0
                ):
                    continue
    ...
    def optimize_image_attachments_to_webp(
        self, noteId: str, quality: int = 90, skip_webp=True
    ):
        """
    ...
    def auto_create_internal_link(
        ...
        all_notes = self.search_note(
            search="note.title %= '.*' #!ignoreAutoInternalLink"
        )
        all_note_title_list = []
        ...
        for note_id in tqdm(target_notes):
            ...
            if verbose:
                logger.info(
                    f"current note id: {note_id} title: {current_note['title']}"
                )
            ...
            tmp_list_for_current_note = (
                processed_child_note_title_list + processed_note_title_list
            )
    ...
    def traverse_note_tree(
        ...
        if len(search_result) >= limit:
            logger.info(
                f"Reached limit of {limit} notes using {method} method, stopping traversal"
            )
...
class ListTemplate(string.Template):
    ...
    def substitute(
        self, mapping: Optional[Mapping[str, object]] = None, **kwds: object
    ) -> str:
...
class ItemTemplate(string.Template):
    ...
    def __init__(
        self, description: Optional[str] = None, checked: bool = False
    ) -> None:
        super().__init__(
